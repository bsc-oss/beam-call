...
vitest.mock("@livekit/components-core", async (importOriginal) => ({
 ...
 * a noise every time.
 */
// PG_CHANGED: join sound plays when media becomes available, not just on membership join.
test("plays one sound when entering a call", () => {
  const remoteParticipants$ = new BehaviorSubject<RemoteParticipant[]>([]);
  const participantsSpy = vitest
    .spyOn(ComponentsCore, "connectedParticipantsObserver")
    .mockReturnValue(remoteParticipants$);
  const mediaSpy = vitest
  ...
  // First add memberships, then simulate media becoming available.
  act(() => {
    rtcMemberships$.next([localRtcMember, aliceRtcMember, bobRtcMember]);
  });
  // No sound yet — media not available.
  expect(playSound).not.toHaveBeenCalled();

  // Remote participants connect (media becomes available).
  act(() => {
    remoteParticipants$.next([aliceParticipant, bobParticipant]);
  });
...
test("plays a sound when a user joins", () => {
  ...
  // Remote participants connect — triggers join sound.
  act(() => {
    remoteParticipants$.next([aliceParticipant, bobParticipant]);
  });
