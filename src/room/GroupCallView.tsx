...
export const GroupCallView: FC<Props> = ({
  ...
  useEffect(() => {
    ...
    void rtcSession.updateCallIntent(videoEnabled ? "video" : "audio");
  }, [rtcSession, joined, videoEnabled]);
  ...
  const onLeft = useCallback(
    (
      ...
      setJoined(false);
      setLeft(true);
      // PG_CHANGED: When the audioContext is broken, which happens sometimes,
      // for unknown reasons (see issue #7 of pg-call repo),
      // the playSound will 'block' the flow of code, so the webview doesn't
      // close. So we make it so we don't wait more than 1 second for this
      // this promise to resolve
      const audioPromise = Promise.race([
        (async (): Promise<void> => {
          try {
            await leaveSoundContext.current?.playSound(playSound);
          } catch (e) {
            logger.warn("Failed to play leave sound", e);
          }
        })(),
