...
// PG_CHANGED: useMediaDevices, * as controls, InlineSpinner
...
export const RoomPage: FC = () => {
  ...
  // PG_CHANGED: Eagerly create the microphone track in the background, so we don't have to wait for it when we actually need it.
  // Also, we need it early so we route the audio track in time to the earpice/speaker so we can start playing the waiting tone.
  ...
  useEffect(() => {
    mediaDevices
      ...
      .catch((error) => {
        logger.error("Failed to create local microphone track", error);
      });
  }, [mediaDevices]);
  ...
  const groupCallView = (): JSX.Element => {
    switch (groupCallState.kind) {
      ...
      case "loading":
        return (
          <FullScreenView>
            {/* PG_CHANGED: show loading indicator to be more consistent with other loading state */}
