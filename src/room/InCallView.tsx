...
// PG_CHANGED: remove IconButton & Tooltip
...
import {
  ...
  // PG_CHANGED: add AudioOutputButton
  AudioOutputButton,
} from "../button";
...
import ringtoneMp3 from "../sound/waiting_tone.mp3?url";
import ringtoneOgg from "../sound/waiting_tone.ogg?url";
...
export const ActiveCall: FC<ActiveCallProps> = (props) => {
  ...
  const onLocalAudioTrackReady = useCallback(() => {
    // PG_CHANGED: callback to native that playback channel is ready can only by done
    // local microphone track is initialized (in useECConnectionState.ts)
    // Otherwise audio will not be routed properly to EARPIECE or SPEAKER
...
export const InCallView: FC<InCallViewProps> = ({
  ...
  // PG_CHANGED: moved code up (unchanged)
  const earpieceMode = useBehavior(vm.earpieceMode$);
  const audioOutputSwitcher = useBehavior(vm.audioOutputSwitcher$);

  // PG_CHANGED: turn on speaker when camera is turned on
  const toggleCamera = useCallback(() => {
    muteStates.video.setEnabled?.((e) => {
      const newState = !e;
      // If enabling video while in earpiece mode, switch to speaker
      if (newState && earpieceMode && audioOutputSwitcher) {
        audioOutputSwitcher.switch();
      }
      return newState;
    });
  }, [muteStates, earpieceMode, audioOutputSwitcher]);
  ...
  // PG_CHANGED: remove top-bar speaker/earpiece switch
  ...
  // PG_CHANGED: remove earpiece-overlay
  ...
  buttons.push(
    ...
    />,
    // PG_CHANGED: add eapiece/speaker switch to bottom-bar
    <AudioOutputButton
      key="audio_output"
      speakerActive={audioOutputSwitcher?.targetOutput === "speaker"}
      onClick={(e) => {
        e.preventDefault();
        audioOutputSwitcher?.switch();
      }}
    ...
    >
      {headerStyle !== "none" && (
        <div className={styles.logo}>
          {/* PG_CHANGED: add our own logo's */}
          <LogoMark width={24} height={24} aria-hidden />
          <LogoType
            width={80}
            height={11}
            aria-label={import.meta.env.VITE_PRODUCT_NAME || "Beam Call"}
          />
