...
// PG_CHANGED: useLocalMicrophoneTrack
import { useLocalMicrophoneTrack } from "../MediaDevicesContext.ts";
...
async function doConnect(
  ...
  try {
    /// PG_CHANGED: we use our pre-created microphone track instead

    if (!preCreatedAudioTrack) {
      logger.info("Pre-created audio track was null");
    }

    preCreatedAudioTrack = localAudioTrack;
...
export function useECConnectionState(
  ...
  // PG_CHANGED: our pre-created local microphone track
