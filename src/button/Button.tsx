...
// PG_CHANGED: move speaker button to bottom-bar
...
export const AudioOutputButton: FC<AudioOutputButtonProps> = ({
  ...
  const { t } = useTranslation();
  const Icon = VolumeOnSolidIcon; // Always use speaker icon
  const label = t("settings.devices.loudspeaker");
  ...
  return (
    <Tooltip label={label}>
      <CpdButton
        iconOnly
        Icon={Icon}
        kind={speakerActive ? "secondary" : "primary"}
        {...props}
      />
    </Tooltip>
  );
