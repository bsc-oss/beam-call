...
import {
  combineLatest,
  // PG_CHANGED: distinctUntilChanged
...
// PG_CHANGED: createLocalAudioTrack, type LocalAudioTrack
  ...
  implements MediaDevice<AudioOutputDeviceLabel, SelectedAudioOutputDevice>
{
  ...
  public readonly available$ = this.scope.behavior(
    combineLatest(
      ...
      (availableRaw, iosDeviceMenu) => {
        let available: Map<string, AudioOutputDeviceLabel> = new Map(
          availableRaw.map(
        ...
        if (iosDeviceMenu && availableRaw.some((d) => d.forEarpiece)) {
          // PG_CHANGED: makes the virtual earpiece device the first entry
          available = new Map([
  ...
  public readonly selected$ = this.scope.behavior(
    combineLatest(
      ...
      // PG_CHANGED: previously this was emitted multiple times unnecessarily
    ).pipe(
      distinctUntilChanged<SelectedAudioOutputDevice | undefined>(
        (prev, curr) =>
          prev?.id === curr?.id &&
          prev?.virtualEarpiece === curr?.virtualEarpiece,
      ),
    ),
  );
...
export class MediaDevices {
  ...
  private _localMicrophoneTrack: LocalAudioTrack | null = null;

  /**
   * PG_CHANGED
   * Gets or creates the local microphone track.
   */
  public async getLocalMicrophoneTrack(): Promise<LocalAudioTrack> {
    if (!this._localMicrophoneTrack) {
      const selectedDevice = this.audioInput.selected$.value;
      this._localMicrophoneTrack = await createLocalAudioTrack({
        deviceId: selectedDevice?.id,
      });
    }
    return this._localMicrophoneTrack;
  }
