...
export interface RemoteUserMediaViewModel
  ...
  /**
   * PG_CHANGED
   * Whether this user's media is available: the LiveKit room exists and the
   * participant has connected. This is used to trigger the join sound effect
   * at the right moment — unlike waitingForMedia$ which is false both when
   * there is no room at all AND when media is fully connected.
   */
  mediaAvailable$: Behavior<boolean>;
}
