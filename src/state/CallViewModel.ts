...
import {
  ...
  // PG_CHANGED: RoomEvent, Track, type RemoteTrack, type RemoteTrackPublication
...
export class CallViewModel extends ViewModel {
  ...
  /**
   * PG_CHANGED: Observable that emits when a remote participant's audio track is subscribed
   */
  public readonly audioTrackConnected$ = new Subject<void>();

  // PG_CHANGED: only emit sound when audioTrackConnected is triggered, which happens when an audio track is subscribed
  public readonly userCountHistory$ = this.userMedia$.pipe(
  ...
  public readonly joinSoundEffect$ = this.audioTrackConnected$.pipe(
    withLatestFrom(this.userCountHistory$),
    filter(
      ([_, userCount]) =>
        userCount.current <= MAX_PARTICIPANT_COUNT_FOR_SOUND &&
        userCount.current > userCount.previous,
    ),
    map(() => {}),
  ...
  /**
   * PG_CHANGED: track n° of remote participants.
   */
  public readonly remoteParticipantCount$ = this.memberships$.pipe(
    map((changes) => {
      if (!this.userId) {
        logger.warn(
          "Could not access user ID to compute remoteParticipantCount",
        );
        return 0;
      }

      const remoteParticipantCount = [...changes].filter(
        (id) => id.sender !== this.userId,
      ).length;
      return remoteParticipantCount;
    }),
    distinctUntilChanged(),
  );
  ...
  public constructor(
    ...
    // PG_CHANGED: livekit event listener for when livekit auto-track is connected
    this.livekitRoom.on(
      LivekitRoomEvent.TrackSubscribed,
      this.handleAudioTrackSubscribed,
    );
  }

  // PG_CHANGED: livekit event listener handler for when livekit auto-track is connected
  private readonly handleAudioTrackSubscribed = (
    track: RemoteTrack,
    publication: RemoteTrackPublication,
  ): void => {
    if (publication.source === Track.Source.Microphone) {
      this.audioTrackConnected$.next();
    }
  };

  // PG_CHANGED: remove livekit event listener for when livekit auto-track is connected
