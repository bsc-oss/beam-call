  ...
  const joinSoundEffect$ = userMedia$.pipe(
    ...
    switchMap((current) => {
      const remotes = current.filter(
        (
          media,
        ): media is WrappedUserMediaViewModel & RemoteUserMediaViewModel =>
          !media.local,
      );
      if (remotes.length === 0) return of(0);
      return combineLatest(remotes.map((media) => media.mediaAvailable$)).pipe(
        map((avail) => avail.filter(Boolean).length),
      );
    }),
    pairwise(),
