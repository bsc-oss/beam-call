...
export function createCallViewModel$(
  ...
  // PG_CHANGED: play sound after media is available, not on join.
  const joinSoundEffect$ = userMedia$.pipe(
    filter((current) => current.length <= MAX_PARTICIPANT_COUNT_FOR_SOUND),
    // Track how many remote participants have available media and play
    // a sound each time that count increases.
    switchMap((current) => {
      const remotes = current.filter(
        (media) => media.vm instanceof RemoteUserMediaViewModel,
      );
      if (remotes.length === 0) return of(0);
      return combineLatest(
        remotes.map(
          (media) => (media.vm as RemoteUserMediaViewModel).mediaAvailable$,
        ),
      ).pipe(map((avail) => avail.filter(Boolean).length));
    }),
    pairwise(),
  ...
  // PG_CHANGED
  // Auto-switch to loudspeaker when camera is enabled.
  // The user can still manually switch to earpiece while the camera is on,
  // but re-enabling the camera will always reset to loudspeaker.
  muteStates.video.enabled$
    ...
    .subscribe(() => {
      if (!earpieceMode$.value) return;
      const available = mediaDevices.audioOutput.available$.value;
      const speaker = [...available].find(([, d]) => d.type === "speaker");
      if (speaker) mediaDevices.audioOutput.select(speaker[0]);
    });
