...
export class Publisher {
  ...
  /**
   * PG_CHANGED
   * Tracks which sources currently have an enabling handler in-flight.
   * When a handler enables a source (e.g. setCameraEnabled(true)), it may
   * create a new track which fires LocalTrackPublished before the handler
   * returns. At that point enabled$.value is still stale (false), so the
   * re-sync in onLocalTrackPublished would undo the enable. We skip the
   * re-sync only for sources being enabled, not disabled — a disabling
   * handler never creates a new track.
   */
  private readonly handlerEnabling = new Set<Track.Source>();
  ...
  private onLocalTrackPublished(
    ...
    // PG_CHANGED
    // Re-apply the mute state only if no enabling handler is currently
    // in-flight for this source. When an enabling handler IS running
    // (e.g. setCameraEnabled(true)), it creates a track which triggers
    // this callback before the handler returns — but enabled$.value is
    // still stale (false), so reading it would undo the enable.
    if (this.handlerEnabling.has(localTrackPublication.source)) return;
  ...
  private observeMuteStates(): void {
    ...
    this.muteStates.audio.setHandler(async (enable) => {
      // PG_CHANGED
      if (enable) this.handlerEnabling.add(Track.Source.Microphone);
      try {
        this.logger.debug(
        ...
        this.handlerEnabling.delete(Track.Source.Microphone);
      }
      return lkRoom.localParticipant.isMicrophoneEnabled;
    });
    this.muteStates.video.setHandler(async (enable) => {
      // PG_CHANGED
      if (enable) this.handlerEnabling.add(Track.Source.Camera);
      try {
        ...
        this.handlerEnabling.delete(Track.Source.Camera);
      }
