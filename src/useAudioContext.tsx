...
// PG_CHANGED: useCallback, useMemo
...
export function useAudioContext<S extends string>(
  ...
  // PG_CHANGED: implement useCallback and useMemo to prevent re-renders.
  // when this component re-renders. This caused it so that the sounds (eg. waiting sound) were not played until the parent component
  // was stable and didn't re-render anymore
  // Fixing this will allow the waiting sound to play much earlier than before providing better feedback to the user.

  const playSoundCallback = useCallback(
    async (name: S): Promise<void> => {
      if (!audioBuffers || !audioBuffers[name]) {
  ...
  const playSoundLoopingCallback = useCallback(
    (name: S, delayS: number): (() => Promise<void>) => {
      if (!audioBuffers || !audioBuffers[name]) {
        throw Error(`Tried to play a sound that wasn't buffered (${name})`);
      }
      if (!audioContext) {
        throw Error("Audio context not ready");
      }
  ...
  const soundDurationMemo = useMemo(
    () =>
      audioBuffers
        ? Object.fromEntries(
  ...
  const result = useMemo(
    () => ({
      ...
      playSoundLooping: playSoundLoopingCallback,
      soundDuration: soundDurationMemo,
    }),
    [playSoundCallback, playSoundLoopingCallback, soundDurationMemo],
  );
