...
function build_assets() {
	...
	pushd ../..  > /dev/null
	# PG_CHANGED: must use the embedded build (vite-embedded.config) so assets use
	# relative URLs (base: "./") and config.json is generated. A plain `pnpm build`
	# uses the default config (base: "/"), producing absolute "/assets/..." and
	# "/config.json" paths that the Android WebViewAssetLoader cannot resolve,
	# resulting in a white screen and FileNotFoundException for config.json/assets.
	pnpm build:embedded
	popd  > /dev/null
}
